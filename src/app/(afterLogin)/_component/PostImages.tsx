...
export default function PostImages({ post }: Props) {
  ...
  if (post.Images.length === 1) {
    return (
      ...
      // </Link>
      <Link
        href={`/${post.memberId}/status/${post.id}/photo/${post.Images[0].imageId}`}
        className={cx(style.postImageSection, style.oneImage)}
        style={{
          backgroundImage: `url(${post.Images[0]?.link})`,
          backgroundSize: "contain",
  ...
  if (post.Images.length === 2) {
    return (
      <div className={cx(style.postImageSection, style.twoImage)}>
        <Link
          href={`/${post.id}/status/${post.memberId}/photo/${post.Images[0].imageId}`}
          style={{
            backgroundImage: `url(${post.Images[0]?.link})`,
            backgroundSize: "cover",
          }}
        ></Link>
        <Link
          href={`/${post.id}/status/${post.memberId}/photo/${post.Images[1].imageId}`}
          style={{
            backgroundImage: `url(${post.Images[1]?.link})`,
            backgroundSize: "cover",
          }}
    ...
    return (
      <div className={cx(style.postImageSection, style.threeImage)}>
        ...
        <div>
          ...
          ></Link>
          <Link
            href={`/${post.id}/status/${post.memberId}/photo/${post.Images[2].imageId}`}
            style={{
              backgroundImage: `url(${post.Images[2]?.link})`,
              backgroundSize: "cover",
            }}
  ...
  if (post.Images.length === 4) {
    return (
      <div className={cx(style.postImageSection, style.fourImage)}>
        ...
        ></Link>
        <Link
          href={`/${post.id}/status/${post.memberId}/photo/${post.Images[3].imageId}`}
          style={{
            backgroundImage: `url(${post.Images[3]?.link})`,
            backgroundSize: "cover",
          }}
